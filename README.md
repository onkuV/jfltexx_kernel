This kernel is for jfltexx-lineage-OS-18.1 which has been modified but
still has original config for build, Build Instructions are given below,
or you can use the official Linux Kernel 3.x release build instruction
...
```
Where `-b lineage-18.1` assures us the correct version (the correct branch in git speaking) of LOS, the 18.1.
...
```bash
git clone https://github.com/DevSH01/arm-linux-androideabi-4.8 toolchain
```
...
make O=../kernel ARCH=arm VARIANT_DEFCONFIG=jf_eur_defconfig lineageos_jf_defconfig menuconfig
Where:
 ...
 * The `lineageos_jf_defconfig` is the config base for LOS
 * The `manuconfig` start the graphical configurator
When you have finished the work, save the config to `.config`.

This will create the default .config file for variant jf_eur_defconfig

### Build the new kernel
The last step: building the kernel!

There are some little things to consider the kernel name.

#### Kernel name
The string that represent the kernel name (e.g. "Linux Name 3.4.112-gb6ef1a5f759-dirty #4 SMP PREEMPT Mon Sep 30 22:44:34 CEST 2022 armv7l") is generated automatically, based on some enviromental variables and must be *unique*. In particular, the alphanumeric string followed by ***-dirty*** is based on the git status of our repository.
...
```bash
  ...
  ```
 Our new custom kernel for LineageOS 18.1 is stored in ***kernel/arch/arm/boot/zImage*** and can be flashed on our Samsung Galaxy S4 (GT-I9505 jflte).
